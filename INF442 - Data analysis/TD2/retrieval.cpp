...
int linear_scan(point q, int dim, point* P, int n) {
    // Exercise 2
    int idx = 0;  // It will contain the index of the closest point
    double min_dist = dist(q, P[0], dim);

    for (int i=0; i < n; ++i) {
        double current_dist = dist(q, P[i], dim);
...
double compute_median(point* P, int start, int end, int c) {
    // Exercise 3
    double coordinates[end - start];
    for (int i = 0; i < end - start; ++i)
        coordinates[i] = P[i + start][c];

    std::sort(coordinates, &(coordinates[end - start]));

    return coordinates[(end - start)/2];
}
...
int partition(point* P, int start, int end, int c) {
    ...
    int idx = (end + start)/2;  // this is where we store the index of the median
...
node* create_node(int _c, double _m, int _idx, node* _left, node* _right) {  
  // Exercise 5
    ...
    internal_node->c = _c;
    internal_node->m = _m;
    internal_node->idx = _idx;
    internal_node->left = _left;
    internal_node->right = _right;
